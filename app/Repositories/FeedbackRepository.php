...
WHERE complaint_id = $complaint_id  and type like 'image'"));
     ...
     * @return mixed
     */
    public function getComplaintVideo(int $complaintId = 0)
    {
        return DB::select(DB::raw("SELECT *
FROM media
  INNER JOIN complaint_media ON media.id = complaint_media.media_id
WHERE complaint_id = $complaintId  and type like 'video'"));
    }
...
WHERE name LIKE '%-task%';"));
     ...
     * @return mixed
     */
    public function assignComplaintMedia($complaint_id = 0, $media_id = 0, $extension = '', $time = '')
    {
...
"));
     ...
     * @return mixed
     */
    public function deleteComplaintMediaExcept(int $complaint_id, string $exception)
    {
        return ComplaintMedia::where('complaint_id', '=', $complaint_id)->whereRaw('media_id not in (' . $exception . ')')->delete();
    }
     ...
     * @return mixed
     */
    public function updateComplaint(int $complaint_id, int $task, string $date, string $complaint)
    {
        return Complaint::where('id', '=', $complaint_id)->update([
            'task_id' => $task,
            'date' => $date,
            'complaint' => $complaint
        ]);
