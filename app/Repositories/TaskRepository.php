...
ORDER BY schedules.start_time DESC;"));
     ...
     * @return mixed
     */
    public function getTaskByClient($client_id = 0)
    {
        return DB::select(DB::raw("SELECT
  tasks.id                                                                                             AS task_id,
  tasks.name                                                                                           AS task_name,
...
FROM tasks
  ...
  INNER JOIN cleaners ON cleaner_task.cleaner_id = cleaners.id
  INNER JOIN inspectors ON inspector_task.inspector_id = inspectors.id WHERE cleaners.deleted_at IS NULL
  AND client_id = ".$client_id." AND clients.deleted_at IS NULL AND inspectors.deleted_at IS NULL 
ORDER BY schedules.start_time DESC;"));
    }
     ...
     * @return mixed
     */
    public function getCleanerTimes($client_id = 0)
    {
        return DB::select(DB::raw("SELECT
            tasks.id                                                                  AS task_id,
            tasks.name                                                                AS task_name,
            clients.id                                                                AS client_id, 
            cleaner_task.start_date                                                   AS start_date, 
            cleaner_task.end_date                                                     AS end_date, 
            cleaners.first_name                                                       AS cleaner_first_name,
            cleaners.last_name                                                        AS cleaner_last_name 
            FROM tasks
            INNER JOIN clients ON tasks.client_id = clients.id 
            INNER JOIN cleaner_task ON tasks.id = cleaner_task.task_id 
            INNER JOIN cleaners ON cleaner_task.cleaner_id = cleaners.id
            WHERE cleaners.deleted_at IS NULL AND client_id = ".$client_id."
        "));
    }
...
//        }
    ...
    {
        return DB::select(DB::raw("SELECT cleaners.*,tasks.*,schedules.*
FROM cleaners
...
where task_id = $task_id
  and status like 'FINISHED'"));
     ...
     * @return mixed
     */
    public function getUpcomingTasks(string $now)
    {
        return DB::select(DB::raw("select tasks.*,
       clients.name                                             as client,
       concat(cleaners.first_name, ' ', cleaners.last_name)     as cleaner,
       concat(inspectors.first_name, ' ', inspectors.last_name) as inspector,
       schedules.*
from tasks
         inner join schedule_task on tasks.id = schedule_task.task_id
         inner join schedules on schedule_task.schedule_id = schedules.id
         ...
         inner join cleaners on cleaner_task.cleaner_id = cleaners.id
         inner join inspectors on inspector_task.inspector_id = inspectors.id
where schedules.`repeat` is true
   or (schedules.`repeat` is not true and schedules.start_time > '$now')
order by hour(start_time) asc;"));
    ...
    {
        return DB::select(DB::raw("select tasks.name,
       concat(cleaners.first_name, ' ', cleaners.last_name)     as cleaner,
       concat(inspectors.first_name, ' ', inspectors.last_name) as inspector,
       clients.name                                             as client,
       schedules.*,
...
"));
    ...
    {
        return DB::select(DB::raw("select clients.name                                             as client,
       concat(cleaners.first_name, ' ', cleaners.last_name)     as cleaner,
       concat(inspectors.first_name, ' ', inspectors.last_name) as inspector,
       task_item_status.status,
       task_items.name                                          as name
from tasks
         inner join clients on tasks.client_id = clients.id
         inner join cleaner_task on tasks.id = cleaner_task.task_id
         inner join inspector_task on tasks.id = inspector_task.task_id
         ...
         inner join task_items on tasks.id = task_items.task_id
         left join task_item_status on task_items.id = task_item_status.task_item_id
where tasks.name in (select concat(ticket, '-task') from complaints)
  and task_item_status.status is null;"));
    }
     ...
     * @return mixed
     */
    public function getTaskStatus(int $task_id, int $cleaner_id)
    {
        return DB::select(DB::raw("select task_status.status
from task_status
         inner join cleaner_schedules on task_status.cleaner_schedule_id = cleaner_schedules.id
where task_id = $task_id
  ...
  and status like 'FINISHED'"));
    ...
    /**
     * @param int $cleanerScheduleId
     * @return mixed
     */
    public function getTaskStatusByCleanerSchedule(int $cleanerScheduleId)
    {
        return DB::select(DB::raw("select *
from task_status
where cleaner_schedule_id = $cleanerScheduleId;"));
    }
}